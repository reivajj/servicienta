...
async function main() {
  ...
  const logger = createLogger('seed-technician-profiles');
  const supabase = createSeedSupabaseClient(env);
  const result = await seedTechnicianProfiles({
    context: { env, logger, supabase },
    technicianLimit: readNumberFlag(args, 'technicians') ?? 10,
  });

  logger.info('Technician profiles seed finished', result);
}
